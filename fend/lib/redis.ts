...
/**
 * Store user link data using the HitLimitedCache
 * @param userLinkData User data to store
 * @param randomKey Optional custom key to use (will generate UUID if not provided)
 * @param expiresIn Expiration time in seconds (default: 10)
 * @returns The random key used to store the data
 */
export async function storeUserLinkData(
  userLinkData: IUserLink,
  randomKey?: string,
  expiresIn: number = 10
) {
  try {
    // Use provided randomKey or generate a new UUID
    const key = randomKey || randomUUID();

    // Use the HitLimitedCache to store the user data
    await hitLimitedCache.set<IUserLink>(
      `user_link_data:${key}`,
      userLinkData,
      expiresIn
    );
    ...
    console.error(error);
    throw new Error("fail_to_store_user_link_data");
  }
}

/**
 * Get user link data using the HitLimitedCache
 * @param randomKey The key for retrieving the user link data
 * @returns The user link data or null if not found
 */
export async function getUserLinkData(randomKey: string) {
  try {
    // Use the HitLimitedCache to retrieve the user data
    const userLinkData = await hitLimitedCache.get<IUserLink>(
      `user_link_data:${randomKey}`
    );
    return userLinkData;
  } catch (error) {
    console.error(error);
    throw new Error("user_link_data_not_found");
  }
}
